...
import correctSoundFile from "./assets/correctSound.mp3"
import wrongSoundFile from "./assets/wrongSound.mp3"
...
export default function GNGContainer() {
  ...
  const toggleMute = () => {
    ...
    if (correctSoundRef.current) correctSoundRef.current.muted = newMuted
    if (wrongSoundRef.current) wrongSoundRef.current.muted = newMuted
  }
  ...
  const handleGuess = (letter) => {
    if (getCharLang(letter) === lang) {
      if (lettersList.some(item => item.letter === letter)) {
        ...
        if (wrongSoundRef.current) wrongSoundRef.current.play().catch(() => {})
      }
    } else {
      alert(lang === "en"
