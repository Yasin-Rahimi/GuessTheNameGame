    ...
    const renderCards = () => {
        return lettersList.map((letter) => (
            <div
                ...
                ${
                    letter.shown
                        ? "bg-emerald-500 animate-[pop_0.2s_ease-out]"
                        : "bg-gray-400"
                }`}
            >
    ...
    const renderWrongCars = () => {
        return wrongLetters.map((letter, index) => (
            <div
                style={{ fontFamily: "Vazirmatn" }}
                key={index}
                className="bg-red-500 p-4 w-14 h-16 flex items-center justify-center m-2 rounded-2xl 
                text-white font-bold shadow-md text-xl
